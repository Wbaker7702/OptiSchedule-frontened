        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3 space-y-2">
             <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-xs transition-all text-left ${activeSection === 'security' ? 'bg-white border border-blue-600/20 text-blue-600 shadow-sm' : 'hover:bg-white text-gray-500'}`}
             >
                <Shield className="w-4 h-4" /> Security Protocol
             </button>
             <button 
                onClick={() => setActiveSection('erp')}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-xs transition-all text-left ${activeSection === 'erp' ? 'bg-white border border-blue-600/20 text-blue-600 shadow-sm' : 'hover:bg-white text-gray-500'}`}
             >
                <Database className="w-4 h-4" /> ERP & CRM Ingress
             </button>
             <button 
                onClick={() => setActiveSection('visual')}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-xs transition-all text-left ${activeSection === 'visual' ? 'bg-white border border-blue-600/20 text-blue-600 shadow-sm' : 'hover:bg-white text-gray-500'}`}
             >
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <div>
                             ...
                             <div className="flex bg-gray-100 p-1 rounded-xl border border-gray-200">
                                {(['Standard', 'High', 'Absolute'] as SecurityLevel[]).map(level => (
                                   <button 
                                     key={level}
